...
# 📅 Según fechas segmentamos por semestres.
...
# 📉 Frecuencias de mensajes diários.

```{r}
# PALETA DE COLORES
paleta.semestres <- brewer.pal(8,"Set1")[c(7,5,1,3,4,2,6,8)]# VERIFICANDO CUÁNTOS MENSAJES SE ENVIARON DURANTE EL PERIODO DE TIEMPO
miChat %>% 
  group_by(semestre) %>% 
  count(day) %>%
...
```
# 🛠 ¿Agrupación para cantidad de mensajes por usuario?
...
# 🌄 Graficando el total de mensajes.
...
# 🧍‍♀️ Emojis más utilizados por usuario.
...
# 🙋 Graficamos, las palabras más utilizadas por usuario.
...
# 🙋 Ahora analizaremos el bocavulario más diverso en la conversación.

```{r}
#Quién tiene el léxico más diverso?
miChat %>%
  unnest_tokens(input = text,
                output = word) %>%
  filter(!word %in% remover_palabras) %>%
...
# SELECCIONAR SÓLO PALABRAS QUE NADIE MÁS USA
  ...
  coord_flip() +
  ggtitle("Top de palabras únicas usadas por Mi")
...
# Página de emojis que utilizaremos.
